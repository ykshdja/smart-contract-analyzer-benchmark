...
contract FranceInu is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x7FD3F619bd145c7Fc5aa39bB3eAc63FD65a5dffc),
        pot: payable(0x135a6e2F1D001f0c7099C15cC638485Ea24BF7d4)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 pot) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.pot = pot;
        _ratios.totalSwap = liquidity + marketing + pot;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable pot) external onlyOwner {
        require(marketing != address(0) && pot != address(0), "Cannot be zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.pot = payable(pot);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        bool success;
        uint256 potBalance = (amtBalance * ratios.pot) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - potBalance;
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 55000}("");
        }
        if (ratios.pot > 0) {
            (success,) = _taxWallets.pot.call{value: potBalance, gas: 55000}("");
        }
    }
