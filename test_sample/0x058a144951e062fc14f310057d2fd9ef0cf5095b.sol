...
contract HourglassX {
	...
	function createTokens(address creator, uint256 eth, address referrer, bool reinvestment) internal returns(uint256) {
		...
		if (referrer == NULL_ADDRESS) { referrer = savedReferral[creator]; }
		uint256 tmp = hourglass.balanceOf(address(refHandler));
		if (creator == referrer) { invalidMasternode = true;
		...
		if (tmp < parentReferralRequirement) {
			...
			if (reinvestment) {
				if (address(this).balance < eth && hourglass.myDividends(true) > 0) { hourglass.withdraw(); }
				if (address(this).balance < eth) { refHandler.sendETH(address(this), eth - address(this).balance); }}
